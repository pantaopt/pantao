...
//@property (nonatomic, strong) ptPlayer *ptPlayer;
...
//        [[ptPlayer shareManager] videoURLStr:videoURLStr :self.layer];
        //AVPlayer
...
//    [ptPlayer shareManager].playerLayer.frame = self.bounds;
}
#pragma mark
...
//    AVPlayerItem *playerItem = [ptPlayer shareManager].currentItem;
...
//    if ([ptPlayer shareManager].player.rate != 1.f) {
        if ([self currentTime] == [self duration])
            [self setCurrentTime:0.f];
//        if ([[UIApplication sharedApplication].keyWindow.subviews[[UIApplication sharedApplication].keyWindow.subviews.count-1] isKindOfClass:[self class]]) {
//            [[UIApplication sharedApplication].keyWindow.subviews[[UIApplication sharedApplication].keyWindow.subviews.count-1] removeFromSuperview];
//        }
        [self.player play];
...
//    [ptPlayer shareManager].currentItem = [[ptPlayer shareManager] getPlayItemWithURLString:videoURLStr];
        ...
        [self.currentItem addObserver:self
                           forKeyPath:@"status"
                              options:NSKeyValueObservingOptionInitial | NSKeyValueObservingOptionNew
                              context:PlayViewStatusObservationContext];
//    [[ptPlayer shareManager].currentItem addObserver:self
//                       forKeyPath:@"status"
//                          options:NSKeyValueObservingOptionInitial | NSKeyValueObservingOptionNew
//                          context:PlayViewStatusObservationContext];
    
    [self.player replaceCurrentItemWithPlayerItem:self.currentItem];
//    [[ptPlayer shareManager].player replaceCurrentItemWithPlayerItem:[ptPlayer shareManager].currentItem];
...
//    [[ptPlayer shareManager].player seekToTime:kCMTimeZero completionHandler:^(BOOL finished) {
...
//    [[ptPlayer shareManager].player seekToTime:CMTimeMakeWithSeconds(slider.value, 1)];
...
//                if (CMTimeGetSeconds([ptPlayer shareManager].player.currentItem.duration)) {
                    self.progressSlider.maximumValue = CMTimeGetSeconds(self.player.currentItem.duration);
//                    self.progressSlider.maximumValue = CMTimeGetSeconds([ptPlayer shareManager].player.currentItem.duration);
                }
...
//    if (self.currentTime == self.duration&&[ptPlayer shareManager].player.rate==.0f) {
...
//     if ([ptPlayer shareManager].player.rate==.0f&&self.currentTime != self.duration) {//暂停状态
        //        if (self.bottomView.alpha == 0.0) {
        //
        //        }else{
        //            self.bottomView.alpha = 1.0;
...
//    }else if([ptPlayer shareManager].player.rate==1.0f){
...
//    [[ptPlayer shareManager].player addPeriodicTimeObserverForInterval:CMTimeMakeWithSeconds(interval, NSEC_PER_SEC)  queue:NULL /* If you pass NULL, the main queue is used. */ usingBlock:^(CMTime time){
//        [self syncScrubber];
//    }];
...
//        double time = CMTimeGetSeconds([[ptPlayer shareManager].player currentTime]);
...
- (CMTime)playerItemDuration{
    AVPlayerItem *playerItem = [self.player currentItem];
//    AVPlayerItem *playerItem = [[ptPlayer shareManager].player currentItem];
